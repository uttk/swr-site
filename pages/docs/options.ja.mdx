...
- `options`: （_任意_） この SWR フックのオプションをオブジェクト
...
- `initialData`: 返される初期データ（注：フックごと） [（詳細）](/docs/with-nextjs)
- `revalidateOnMount`: コンポーネントがマウントされている場合の自動再検証を有効または無効にします（デフォルトでは、initialDataが設定されていない場合、マウント時に再検証が行われます。このフラグを使用して動作を強制します）
