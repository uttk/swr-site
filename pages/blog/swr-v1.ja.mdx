...
リソースが変更されない場合は、リソースを**イミュータブル（不変）**として固定したい場合があります。その場合は、自動再検証を無効にして、リクエストを一回だけ行うことをオススメします。また、これを簡単にするためのヘルパーフックがあります：
...
`useSWR` フックとまったく同じ API を備えていますが、タブフォーカスやネットワーク復帰時に再検証されることはありません。また、新しいオプション `revalidateIfStale` もあり、これを使用して動作を正確に制御できます。詳細については、[こちら](/docs/revalidation#disable-automatic-revalidations)をご覧ください。
...
この新機能を使用して、多くの強力な処理を実行できます。ここにいくつかの例があります： [RegEx を使用した複数のキーの変更](/docs/advanced/cache#mutate-multiple-keys-from-regex)、 [LocalStorage を使った永続キャッシュ](/docs/advanced/cache#localstorage-を使った永続キャッシュ)、[テストケース間のキャッシュのリセット](/docs/advanced/cache#テストケース間のキャッシュのリセット)。
...
現在のキャッシュプロバイダーとグローバルな `mutate` 関数を含む、すべてのグローバル設定を返す新しいフック API があります：
...
この機能を使用すると、多くの新しいアイディアを実装できます。いくつかの例を作成しました：[_Request Logger_](/docs/middleware#request-logger), [_Keep Previous Data When Changing the Key_](/docs/middleware#keep-previous-result), and [_Serialize Object Keys_](/docs/middleware#serialize-object-keys).
...
Thanks to our [contributors](https://github.com/vercel/swr-site/graphs/contributors) and Nextra’s [i18n feature](https://nextra.vercel.app/features/i18n), we now offer SWR documentation in five different languages:

コントリビューターと Nextra の i18n 機能のおかげで、 5 つの異なる言語で SWR ドキュメントを提供できるようになりました。
...
### デフォルトの Fetcher はもうありません

SWR は、デフォルトの fetcher （ データを JSON として解析する `fetch` の呼び出し ）を提供しなくなりました。変更を移行する最も簡単な方法は、 `<SWRConfig>` コンポーネントを使用する事です：
...
### フックから返された `mutate` を使用する事をオススメします
