...
<Callout emoji="✅">
  SWR では、 コンポーネントはデータの更新を<strong>継続的</strong>かつ<strong>自動的</strong>に受け取る事が出来ます。
  <br/>
...
`fetcher` はデータを返す任意の非同期関数で、ネイティブの fetch や Axios のようなツールを使うことが出来ます。

このフックは、リクエストの状態に基づいて `data` と `error` の二つの値を返します。
